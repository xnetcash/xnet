...
The xnet Core daemon can be configured to act as a trusted "border
router", implementing the xnet wire protocol and relay, making
...
The ZeroMQ feature in xnet Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building xnetd:
...
For instance:
    ...
    $ xnetd -zmqpubhashtx=tcp://127.0.0.1:249069 \
               -zmqpubhashtx=tcp://192.168.1.2:249069 \
               -zmqpubrawtx=ipc:///tmp/xnetd.tx.raw \
...
These options can also be provided in xnet.conf.
...
From the perspective of xnetd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into xnetd directly. Furthermore, no information is
...
using. xnetd appends an up-counting sequence number to each
